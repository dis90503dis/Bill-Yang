<template>
  <div>
    <div class="contact-info-img relative"></div>
    ...
    <img src="@/section/s2/logo2.svg" class="logo"> -->
      ...
      <div class="flex justify-between w-full contact-item-box no-gap address" v-if="info.address">
        <div class="flex contact-item justify-center items-center add">
          <div>
            <span v-if="info.address1">{{ info.address1 }}：</span>{{ info.address }}
          </div>
        </div>
        <div
  ...
  <div class="modal -mt-20 md:-mt-72">
    <div class="modal-box py-12 relative flex flex-col items-center justify-center">
      ...
      <div class="text-xl mt-4 font-bold">
        {{ modalType == "phone" ? "預約專線" : modalType == "fb" ? "Facebook Messenger" : `${info.address2 ? info.address2 : "導航地址"}` }}
      </div>
      <!-- content -->
      <div class="text-md mt-4">{{ modalType == "phone" ? info.phone : modalType == "fb" ? "線上諮詢" : `${info.address}` }}</div>
      <!-- btn -->
      <div
...
if (!smoothScroll) {
  console.warn("[ContactInfo] smoothScroll not catch provide")
}

// 自己算 isMobile，不要用 this.$isMobile
